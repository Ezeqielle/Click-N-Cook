...
if (isset($_SESSION['id']) AND !empty($_SESSION['id']) AND $_SESSION['administrator'] == 1 AND isset($_GET['idUser'])){
    ...
    if($dataUser['entranceFee'] == 0) {
        $queryUserAdd = $pdo->prepare("UPDATE FRANCHISEE SET entranceFee = TRUE WHERE id = " . $user);
        $queryUserAdd->execute();
    } else {
        $queryUserAdd = $pdo->prepare("UPDATE FRANCHISEE SET entranceFee = FALSE WHERE id = " . $user);
        $queryUserAdd->execute();
    }
