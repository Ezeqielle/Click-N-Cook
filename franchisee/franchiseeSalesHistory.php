...
if(isset($_SESSION['id']) AND !empty($_SESSION['id']) AND ($_SESSION['administrator'] == 0 || $_SESSION['administrator'] == 1)) {
    ...
    $reqQuantityPurchaseDish = $db->query('SELECT count(*) FROM CONTAINSDISHSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ')');
    $rowClientDish = $reqQuantityPurchaseDish->fetch();


    $reqQuantityPurchaseMenu = $db->query('SELECT count(*) FROM CONTAINSMENUSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ')');
    ...
    While ($nbPage <= $nbPageMax) {
        ...
        $year = date("d-m-Y");
        $nbBill = conv(TXT_SALESHISTORY_INVOICE . $year);
        ...
        $reqDish = $db->query('SELECT * FROM CONTAINSDISHSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ') ORDER BY idPurchaseClient LIMIT ' . $minLimit . ' , ' . 18);
        ...
        $reqIdDish = $db->query('SELECT * FROM CONTAINSDISHSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ') ORDER BY idPurchaseClient LIMIT ' . $minLimit . ' , ' . 18);
        ...
        $reqIdDishBis = $db->query('SELECT * FROM CONTAINSDISHSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ') ORDER BY idPurchaseClient LIMIT ' . $minLimit . ' , ' . 18);
        if(($reqIdDishBis->fetchALL()) == NULL || $test == 0) {
            $reqDish = $db->query('SELECT * FROM CONTAINSDISHSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ') ORDER BY idPurchaseClient');
            ...
            $reqIdMenu = $db->query('SELECT * FROM CONTAINSMENUSALECLIENT WHERE (SELECT idFranchisee FROM PURCHASECLIENT WHERE idPurchaseClient = bill_numberClient AND idFranchisee = ' . $_SESSION['idFranchiseeSales'] . ') ORDER BY idPurchaseClient LIMIT ' . $minLimitTmp . ' , ' . $maxLimitTmp);
